...
def units_per_electrode_to_mM(E_units, film_thickness_um, kcat_s_inv, electrode_area_mm2=ELECTRODE_AREA_MM2):
...
with tabs[0]:
    ...
    Km_glu_mM = sidebar.slider("Glucose Km (mM)", 0.1, 50.0, 10.0, 0.1)
    kcat_glu = sidebar.slider("Glucose kcat (s⁻¹)", 0.01, 500.0, 100.0, 0.5)
    k3 = sidebar.slider("O₂ reaction rate k3 (M⁻¹ s⁻¹)", 0.01, 1e4, 100.0, 1.0)
    ...
    E_units = sidebar.slider("GOx loading (U per electrode)", 0.01, 10.0, 1.0, 0.01)
...
with tabs[1]:
    ...
    run_sweep = sidebar.button("Run parameter sweep")
        ...
        for i, O2_ppm_val in enumerate(O2_vals_ppm):
            for j, E_units_val in enumerate(E_units_vals):
                ...
                E_tot_mM_sw = units_per_electrode_to_mM(
                    E_units_val, film_thickness_um_sw, kcat_glu_sw
                )
